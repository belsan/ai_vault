...
trustworthy in a way that an unaided model's output is not.
...
that are difficult to synthesise, with the proposals filtered by
chemistry-informed rules. Of the top hundred candidates, thirty-four
matched experimentally synthesised structures that were not in the
...
The common thread runs through every example. None of these is a
naked language-model achievement. Each one is a \emph{system} ---
a model in a loop with executors, verifiers, search procedures, and
human-defined oversight. The model is the engine; the system is
the vehicle.
...
sit close to the closed frontier. Specialised tooling --- agent
...
agent turn began: \citet{yao2023react} formalised the
...
introduced the Model Context Protocol in late~2024, standardising
...
this: a contemporary language model is not, by itself, the artefact
worth designing around. The artefact is the \emph{system} that
wraps the model. Recognising this changes the questions one asks at
...
the agent's behaviour reliable as the surface of the task grows.
...
calling convention, not as a research artefact whose internals we
...
here. Every substantive claim about model behaviour is paired
...
do not have this problem: the artefact the reader downloads today
is the same artefact the book was written against, and the same
artefact a reader running the book in 2030 will be able to run.
...
Chapter~\ref{ch:reliable} are easier to internalise after one has
