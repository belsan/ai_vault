...
The tool catalogue is part of the model's input on every turn. When
...
serialises to between 100 and 300 tokens once the schema is
...
catalogue, and the conversation history each time. A long-running
agent on a 32K-context model can spend a quarter of its window
carrying around the catalogue and the cumulative
...
\subsection{The JSON-serialisation tax}
...
\item \textbf{Subset the tool inventory per task.} A coding agent
  ...
  the model dramatically reduces the steady-state catalogue size.
\item \textbf{Tool retrieval.} Treat the tool catalogue as a
  ...
  catalogue. \citet{patil2023gorilla} formalise this for the case of
...
\item \textbf{Context efficiency.}
  ...
  The catalogue collapses from a fixed prompt overhead into a
...
on the project board, summarise each, and email the summaries to my
...
> tool_call: summarise(text="...issue 1...")
< tool_result: "..."
> tool_call: summarise(text="...issue 2...")
< tool_result: "..."
> tool_call: summarise(text="...issue 3...")
< tool_result: "..."
...
body   = "\n\n".join(summarise(i.description) for i in top3)
...
\item[Prompts.] Pre-authored prompt templates that an agent can
  ...
  request'', ``write a release note'') that an organisation wants
...
years earlier for the Language Server Protocol, which standardised
...
concatenates these into the model's tool catalogue (or, in the
...
organisation, no such server exists, and writing one is a small
project of its own.
...
agent must memorise into a thing the agent looks up the moment it
...
writing an MCP server is hard to amortise. For ten agents across
...
\item \textbf{If the tool is internal, used by one or two agents,
  ...
  schema to keep synchronised, no second deployment story.
...
\item \textbf{When in doubt, start with (3) and migrate to (2)
  ...
  consumer that never materialises is the wasted effort itself.

\end{enumerate}

\noindent
A final caveat applies to both approaches. Neither solves the
\emph{authorisation} problem. Whether the agent reaches
